...
export class CategoryService extends Firestore<ICategory> {
  ...
  private init() {
    this.setCollection(this.CATEGORIES);
  }
  ...
  getAllWithRelations() {
    ...
    return this.userService.getBySession().pipe(
      switchMap(user =>
        this.db
          .collection<ICategory>(this.CATEGORIES, ref => ref.where('createdBy', '==', user.id))
          .snapshotChanges()
          .pipe(
            map(snaps => {
              return snaps.map(s => {
